...
class Field:
    ...
    def G(self, x, u):
        ...
        theta_hat = prev_theta + rot1
        G = np.array([
            [1, 0, -trans * np.sin(theta_hat)],
            [0, 1,  trans * np.cos(theta_hat)],
            [0, 0, 1]
        ])
        return G
    ...
    def V(self, x, u):
        ...
        # YOUR IMPLEMENTATION HERE
        theta_hat = prev_theta + rot1
        sin_theta_hat = np.sin(theta_hat)
        cos_theta_hat = np.cos(theta_hat)

        V = np.array([
            [-trans * sin_theta_hat, cos_theta_hat, 0],
            [ trans * cos_theta_hat, sin_theta_hat, 0],
            [1, 0, 1]
        ])
        return V
